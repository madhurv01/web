    ...
    <section class="auth-bg min-h-[calc(100vh-4rem)] grid md:grid-cols-2">
      <div class="relative hidden md:flex items-center justify-center overflow-hidden">
        ...
        <div class="relative z-10 text-center px-10">
          ...
          ></model-viewer>
          <h2 class="text-2xl font-bold text-white mt-4">Government Access Portal</h2>
          <p class="text-white/70 mt-2 max-w-xs mx-auto">
            Manage citizen complaints, monitor water status and coordinate response teams.
          </p>
        </div>
      </div>
          ...
          <form [formGroup]="form" (ngSubmit)="submit()" class="space-y-4">
            <div>
              ...
              @if (form.get('email')?.invalid && form.get('email')?.touched) {
                <p class="field-error">Enter a valid official email address.</p>
              }
            </div>
            <div>
...
export class GovLoginComponent {
  ...
  async submit() {
    ...
    try {
      ...
      this.errorMessage = friendlyAuthError(e, 'Invalid username or password.');
    } finally {
      this.loading = false;
    }
  }
